...
function getTime(date) {
  return date.toLocaleTimeString('ru-RU', { timeStyle: 'medium' });
}
...
function getNextFriday(date) {
  ...
  const diffToFriday = (7 - date.getDay() + 5) % 7;

  const daysToNextFriday = diffToFriday === 0 ? 7 : diffToFriday;

  date.setDate(currentDate + daysToNextFriday);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return diffInMs / 1000 / 3600 / 24 + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  while (currentDay.getUTCDate() !== 13 || currentDay.getUTCDay() !== 5) {
    currentDay.setUTCDate(currentDay.getUTCDate() + 1);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const normalizeFormatDate = (date) => date.split('-').reverse().join('-');

  const startDate = new Date(normalizeFormatDate(period.start));
  const endDate = new Date(normalizeFormatDate(period.end));
  ...
  const workSchedule = workDays
    .filter((workDate) => workDate <= endDate)
    .map((workDate) => {
